...
class sms_time_listAdmin(admin.ModelAdmin):
    list_display = ('title', 'sms_id', 'stat', 'creat_date', 'send_date')
    search_fields = ['content']
    list_per_page = 10
    actions = ['send_sms']
    actions_on_top = True
    ...
    def __init__(self, model, admin_site):
        self.form.admin_site = admin_site
        super(sms_time_listAdmin, self).__init__(model, admin_site)
    ...
    def send_sms(self, request, queryset, *arg1, **arg2):
        ...
        for sms_item in queryset:
            ...
            for group_object in sms_item.sms_groups.all():
                for user_object in group_object.user_set.all():
                    try:
                        ...
                        stat = 4
                        errors += e.message
            send_result = str(sms_object.post_time_sms(u'定时群发短信', send_users, sms_item.send_date, sms_item.content))
            if len(send_result) > 3:
                sms_item.sms_id = send_result
                stat = 1
            else:
            ...
            sms_item.save(force_update=True)
            self.message_user(request, send_result)
    send_sms.short_description = u'启动定时短信'
