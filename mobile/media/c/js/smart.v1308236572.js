function capfirst(s){return s.substr(0,1).toUpperCase()+s.substr(1)}
...
if(board==''){board='departures';}})function refreshTransport(data){$('#park_and_rides .section-content').empty()
...
clearTimeout(transportTimer)}});var current_url=window.location.pathname+window.location.search;history.navigationMode='compatible';function to_absolute(url){url=url.split('#')[0]
...
capture_outbound();if(!!(window.history&&history.pushState)){window.addEventListener('popstate',function(e,state){if(current_url!=window.location.pathname+window.location.search){async_load(window.location.href,{},'GET');}},false)}});function submitAutomaticLocation(position,method){$('.location').html('Location found; please wait while we put a name to it.')
