...
{prevPage:d||a("")});g()});return e}function g(){var b=jQuery.event.special.orientationchange.orientation()==="portrait",c=b?screen.height:screen.width,b=Math.max(b?480:320,a(window).height()),c=Math.min(c,b);a(".ui-page-active").css("min-height",c)}function i(b,c){c&&b.attr("data-"+a.mobile.ns+"role",c);b.page()}function h(a){for(;a;){if(a.nodeName.toLowerCase()=="a")break;a=a.parentNode}return a}function l(b){var b=a(b).closest(".ui-page").jqmData("url"),c=s.hrefNoHash;if(!b||!k.isPath(b))b=c;return k.makeUrlAbsolute(b,
c)}var n=a(window),p=a("html"),j=a("head"),k={urlParseRE:/^(((([^:\/#\?]+:)?(?:\/\/((?:(([^:@\/#\?]+)(?:\:([^:@\/#\?]+))?)@)?(([^:\/#\?]+)(?:\:([0-9]+))?))?)?)?((\/?(?:[^\/\?#]+\/+)*)([^\?#]*)))?(\?[^#]+)?)(#.*)?/,parseUrl:function(a){if(typeof a==="object")return a;var a=k.urlParseRE.exec(a),b;a&&(b={href:a[0]||"",hrefNoHash:a[1]||"",hrefNoSearch:a[2]||"",domain:a[3]||"",protocol:a[4]||"",authority:a[5]||"",username:a[7]||"",password:a[8]||"",host:a[9]||"",hostname:a[10]||"",port:a[11]||"",pathname:a[12]||
"",directory:a[13]||"",filename:a[14]||"",search:a[15]||"",hash:a[16]||""});return b||{}},makePathAbsolute:function(a,b){if(a&&a.charAt(0)==="/")return a;for(var a=a||"",c=(b=b?b.replace(/^\/|\/?[^\/]*$/g,""):"")?b.split("/"):[],d=a.split("/"),e=0;e<d.length;e++){var f=d[e];switch(f){case ".":break;case "..":c.length&&c.pop();break;default:c.push(f)}}return"/"+c.join("/")},isSameDomain:function(a,b){return k.parseUrl(a).domain===k.parseUrl(b).domain},isRelativeUrl:function(a){return k.parseUrl(a).protocol===
""},isAbsoluteUrl:function(a){return k.parseUrl(a).protocol!==""},makeUrlAbsolute:function(a,b){if(!k.isRelativeUrl(a))return a;var c=k.parseUrl(a),d=k.parseUrl(b),e=c.protocol||d.protocol,f=c.authority||d.authority,g=c.pathname!=="",j=k.makePathAbsolute(c.pathname||d.filename,d.pathname);return e+"//"+f+j+(c.search||!g&&d.search||"")+c.hash},addSearchParams:function(b,c){var d=k.parseUrl(b),e=typeof c==="object"?a.param(c):c,f=d.search||"?";return d.hrefNoSearch+f+(f.charAt(f.length-1)!=="?"?"&":
"")+e+(d.hash||"")},convertUrlToDataUrl:function(a){var b=k.parseUrl(a);if(k.isEmbeddedPage(b))return b.hash.replace(/^#/,"");else if(k.isSameDomain(b,s))return b.hrefNoHash.replace(s.domain,"");return a},get:function(a){if(a===d)a=location.hash;return k.stripHash(a).replace(/[^\/]*\.[^\/*]+$/,"")},getFilePath:function(b){var c="&"+a.mobile.subPageUrlKey;return b&&b.split(c)[0].split(u)[0]},set:function(a){location.hash=a},isPath:function(a){return/\//.test(a)},clean:function(a){return a.replace(s.domain,
"")},stripHash:function(a){return a.replace(/^#/,"")},cleanHash:function(a){return k.stripHash(a.replace(/\?.*$/,"").replace(u,""))},isExternal:function(a){a=k.parseUrl(a);return a.protocol&&a.domain!==v.domain?!0:!1},hasProtocol:function(a){return/^(:?\w+:)/.test(a)},isEmbeddedPage:function(a){a=k.parseUrl(a);if(a.protocol!=="")return a.hash&&(a.hrefNoHash===v.hrefNoHash||z&&a.hrefNoHash===s.hrefNoHash);return/^#/.test(a.href)}},q=null,m={stack:[],activeIndex:0,getActive:function(){return m.stack[m.activeIndex]},
getPrev:function(){return m.stack[m.activeIndex-1]},getNext:function(){return m.stack[m.activeIndex+1]},addNew:function(a,b,c,d){m.getNext()&&m.clearForward();m.stack.push({url:a,transition:b,title:c,page:d});m.activeIndex=m.stack.length-1},clearForward:function(){m.stack=m.stack.slice(0,m.activeIndex+1)},directHashChange:function(b){var c,e,f;a.each(m.stack,function(a,d){b.currentUrl===d.url&&(c=a<m.activeIndex,e=!c,f=a)});this.activeIndex=f!==d?f:this.activeIndex;c?b.isBack():e&&b.isForward()},
ignoreNextHashChange:!1},o="[tabindex],a,button:visible,select:visible,input",y=[],x=!1,u="&ui-state=dialog",w=j.children("base"),v=k.parseUrl(location.href),s=w.length?k.parseUrl(k.makeUrlAbsolute(w.attr("href"),v.href)):v,z=v.hrefNoHash!==s.hrefNoHash,t=a.support.dynamicBaseTag?{element:w.length?w:a("<base>",{href:s.hrefNoHash}).prependTo(j),set:function(a){t.element.attr("href",k.makeUrlAbsolute(a,s))},reset:function(){t.element.attr("href",s.hrefNoHash)}}:d;a.fn.animationComplete=function(b){return a.support.cssTransitions?
a(this).one("webkitAnimationEnd",b):(setTimeout(b,0),a(this))};a.mobile.updateHash=k.set;a.mobile.path=k;a.mobile.base=t;a.mobile.urlstack=m.stack;a.mobile.urlHistory=m;a.mobile.noneTransitionHandler=function(b,c,d,e){e&&e.removeClass(a.mobile.activePageClass);d.addClass(a.mobile.activePageClass);return a.Deferred().resolve(b,c,d,e).promise()};a.mobile.defaultTransitionHandler=a.mobile.noneTransitionHandler;a.mobile.transitionHandlers={none:a.mobile.defaultTransitionHandler};a.mobile.allowCrossDomainPages=
!1;a.mobile.getDocumentUrl=function(b){return b?a.extend({},v):v.href};a.mobile.getDocumentBase=function(b){return b?a.extend({},s):s.href};a.mobile.loadPage=function(b,c){var e=a.Deferred(),f=a.extend({},a.mobile.loadPage.defaults,c),g=null,j=null,h=k.makeUrlAbsolute(b,s.hrefNoHash);if(f.data&&f.type==="get")h=k.addSearchParams(h,f.data),f.data=d;var m=k.getFilePath(h),l=k.convertUrlToDataUrl(h);f.pageContainer=f.pageContainer||a.mobile.pageContainer;g=f.pageContainer.children(":jqmData(url='"+l+
"')");t&&t.reset();if(g.length){if(!f.reloadPage)return i(g,f.role),e.resolve(h,c,g),e.promise();j=g}f.showLoadMsg&&a.mobile.showPageLoadingMsg();a.ajax({url:m,type:f.type,data:f.data,dataType:"html",success:function(d){var q=a("<div></div>"),p=d.match(/<title[^>]*>([^<]*)/)&&RegExp.$1,n=RegExp("\\bdata-"+a.mobile.ns+"url=[\"']?([^\"'>]*)[\"']?");RegExp(".*(<[^>]+\\bdata-"+a.mobile.ns+"role=[\"']?page[\"']?[^>]*>).*").test(d)&&RegExp.$1&&n.test(RegExp.$1)&&RegExp.$1&&(b=m=k.getFilePath(RegExp.$1));
t&&t.set(m);q.get(0).innerHTML=d;g=q.find(":jqmData(role='page'), :jqmData(role='dialog')").first();p&&!g.jqmData("title")&&g.jqmData("title",p);if(!a.support.dynamicBaseTag){var o=k.get(m);g.find("[src], link[href], a[rel='external'], :jqmData(ajax='false'), a[target]").each(function(){var b=a(this).is("[href]")?"href":a(this).is("[src]")?"src":"action",c=a(this).attr(b),c=c.replace(location.protocol+"//"+location.host+location.pathname,"");/^(\w+:|#|\/)/.test(c)||a(this).attr(b,o+c)})}g.attr("data-"+
a.mobile.ns+"url",k.convertUrlToDataUrl(m)).appendTo(f.pageContainer);i(g,f.role);h.indexOf("&"+a.mobile.subPageUrlKey)>-1&&(g=f.pageContainer.children(":jqmData(url='"+l+"')"));f.showLoadMsg&&a.mobile.hidePageLoadingMsg();e.resolve(h,c,g,j)},error:function(){t&&t.set(k.get());f.showLoadMsg&&(a.mobile.hidePageLoadingMsg(),a("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>"+a.mobile.pageLoadErrorMessage+"</h1></div>").css({display:"block",opacity:0.96,top:n.scrollTop()+100}).appendTo(f.pageContainer).delay(800).fadeOut(400,
function(){a(this).remove()}));e.reject(h,c)}});return e.promise()};a.mobile.loadPage.defaults={type:"get",data:d,reloadPage:!1,role:d,showLoadMsg:!0,pageContainer:d};a.mobile.changePage=function(c,g){if(typeof g!=="object"){var h=null;if(typeof c==="object"&&c.url&&c.type)h={type:c.type,data:c.data,forcePageLoad:!0},c=c.url;var j=arguments.length;if(j>1){var l=["transition","reverse","changeHash","fromHashChange"],q;for(q=1;q<j;q++){var n=arguments[q];typeof n!=="undefined"&&(h=h||{},h[l[q-1]]=n)}}if(h)return a.mobile.changePage(c,
h)}if(x)y.unshift(arguments);else{x=!0;var o=a.extend({},a.mobile.changePage.defaults,g);o.pageContainer=o.pageContainer||a.mobile.pageContainer;if(typeof c=="string")a.mobile.loadPage(c,o).done(function(b,c,d,e){x=!1;c.duplicateCachedPage=e;a.mobile.changePage(d,c)}).fail(function(){x=!1;b(!0);e();o.pageContainer.trigger("changepagefailed")});else{var v=o.pageContainer,h=a.mobile.activePage,j=c.jqmData("url");k.getFilePath(j);l=m.getActive();q=m.activeIndex===0;var s=0,n=document.title,t=o.role===
"dialog"||c.jqmData("role")==="dialog";v.trigger("beforechangepage");if(h&&h[0]===c[0])x=!1,v.trigger("changepage");else{i(c,o.role);o.fromHashChange&&m.directHashChange({currentUrl:j,isBack:function(){s=-1},isForward:function(){s=1}});a(document.activeElement||"").add("input:focus, textarea:focus, select:focus").blur();t&&l&&(j=l.url+u);if(o.changeHash!==!1&&j)m.ignoreNextHashChange=!0,k.set(j);(t=c.jqmData("title")||c.children(":jqmData(role='header')").find(".ui-title").text())&&n==document.title&&
(n=t);s||m.addNew(j,o.transition,n,c);document.title=m.getActive().title;a.mobile.activePage=c;o.transition=o.transition||(s&&!q?l.transition:d)||(o.role==="dialog"?a.mobile.defaultDialogTransition:a.mobile.defaultPageTransition);o.reverse=o.reverse||s<0;f(c,h,o.transition,o.reverse).done(function(){b();o.duplicateCachedPage&&o.duplicateCachedPage.remove();p.removeClass("ui-mobile-rendering");e();v.trigger("changepage")})}}}};a.mobile.changePage.defaults={transition:d,reverse:!1,changeHash:!0,fromHashChange:!1,
role:d,duplicateCachedPage:d,pageContainer:d};a("form").live("submit",function(b){var c=a(this);if(a.mobile.ajaxEnabled&&!c.is(":jqmData(ajax='false')")){var d=c.attr("method"),e=k.makeUrlAbsolute(c.attr("action"),l(c));target=c.attr("target");!k.isExternal(e)&&!target&&(a.mobile.changePage(e,{type:d.length&&d.toLowerCase()||"get",data:c.serialize(),transition:c.jqmData("transition"),direction:c.jqmData("direction"),reloadPage:!0}),b.preventDefault())}});a(document).bind("vclick",function(b){(b=h(b.target))&&
k.parseUrl(b.getAttribute("href")||"#").hash!=="#"&&a(b).closest(".ui-btn").not(".ui-disabled").addClass(a.mobile.activeBtnClass)});a(document).bind("vclick click",function(c){var e=h(c.target);if(e){var f=a(e),g=function(){window.setTimeout(function(){b(!0)},200)};if(f.is(":jqmData(rel='back')"))return window.history.back(),!1;if(a.mobile.ajaxEnabled){var j=l(f),e=k.makeUrlAbsolute(f.attr("href")||"#",j);if(e.search("#")!=-1)if(e=e.replace(/[^#]*#/,""))e=k.isPath(e)?k.makeUrlAbsolute(e,j):k.makeUrlAbsolute("#"+
e,v.hrefNoHash);else{c.preventDefault();return}var j=f.is("[rel='external']")||f.is(":jqmData(ajax='false')")||f.is("[target]"),i=a.mobile.allowCrossDomainPages&&v.protocol==="file:"&&e.search(/^https?:/)!=-1,j=j||k.isExternal(e)&&!i;q=f.closest(".ui-btn");j?g():(g=f.jqmData("transition"),j=(j=f.jqmData("direction"))&&j==="reverse"||f.jqmData("back"),f=f.attr("data-"+a.mobile.ns+"rel")||d,a.mobile.changePage(e,{transition:g,reverse:j,role:f}),c.preventDefault())}else g()}});n.bind("hashchange",function(){var b=
k.stripHash(location.hash),c=a.mobile.urlHistory.stack.length===0?"none":d;if(!a.mobile.hashListeningEnabled||m.ignoreNextHashChange)m.ignoreNextHashChange=!1;else{if(m.stack.length>1&&b.indexOf(u)>-1)if(a.mobile.activePage.is(".ui-dialog")){var e=function(){b=a.mobile.urlHistory.getActive().page};m.directHashChange({currentUrl:b,isBack:e,isForward:e})}else{m.directHashChange({currentUrl:b,isBack:function(){window.history.back()},isForward:function(){window.history.forward()}});return}b?(b=typeof b===
"string"&&!k.isPath(b)?"#"+b:b,a.mobile.changePage(b,{transition:c,changeHash:!1,fromHashChange:!0})):a.mobile.changePage(a.mobile.firstPage,{transition:c,changeHash:!1,fromHashChange:!0})}});a(document).bind("pagecreate orientationchange",g)})(jQuery);
